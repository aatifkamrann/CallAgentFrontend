...
export default function Schedule() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/scheduler/status`, withAuthHeaders())
      ...
      .then(data => { if (data) setSchedulerEnabled(data.enabled); })
      .catch(() => {});
  }, []);
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
        ...
        <div className={`px-4 py-2 rounded-md text-sm flex items-center gap-2 ${schedulerEnabled ? 'bg-green-500/10 text-green-600 border border-green-500/30' : 'bg-muted text-muted-foreground border border-border'}`}>
          ...
          {schedulerEnabled ? 'Auto-redial is ACTIVE — checking every 60 seconds for overdue retries' : 'Auto-redial is OFF — overdue retries will not be called automatically'}
        </div>
              ...
              {overdueRetries.map(c => (
                <div key={c.id} className="px-5 py-4">
                  <div className="flex items-center justify-between">
                   ...
                   <p className="text-xs text-muted-foreground mt-1">{c.retry_reason || 'Retry scheduled'}</p>
                   {c.final_response && (
