                ...
                <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={settings.retryNoAnswerHours}
                      onChange={e => setSettings(s => ({ ...s, retryNoAnswerHours: Number(e.target.value) }))} min={0.1} step={0.1} max={72} />
                    <p className="text-[10px] text-muted-foreground">Auto-retry after no answer / busy / callback — {settings.retryNoAnswerHours < 1 ? Math.round(settings.retryNoAnswerHours * 60) + ' min' : settings.retryNoAnswerHours + ' hr'}</p>
                  </div>
                  <div className="space-y-2">
                    <Label className="text-sm font-semibold">Non-Customer Retry (hrs)</Label>
                    <Input type="number" value={settings.retryNonCustomerHours}
                      onChange={e => setSettings(s => ({ ...s, retryNonCustomerHours: Number(e.target.value) }))} min={0.1} step={0.1} max={72} />
