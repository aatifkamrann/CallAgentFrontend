...
const AUTO_DIAL_SESSION_KEY = 'lovable_auto_dial_session';
const AUTO_DIAL_SESSION_MAX_AGE_MS = 30 * 60 * 1000;

interface PersistedAutoDialSession {
  running: boolean;
  jobs: Record<string, AutoDialJob>;
  savedAt: number;
}

function saveAutoDialSession(session: PersistedAutoDialSession) {
  try { localStorage.setItem(AUTO_DIAL_SESSION_KEY, JSON.stringify(session)); } catch {}
}

function clearAutoDialSession() {
  try { localStorage.removeItem(AUTO_DIAL_SESSION_KEY); } catch {}
}

function loadAutoDialSession(): PersistedAutoDialSession | null {
  try {
    const raw = localStorage.getItem(AUTO_DIAL_SESSION_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as PersistedAutoDialSession;
    if (!parsed || typeof parsed !== 'object') return null;
    if (!parsed.savedAt || (Date.now() - parsed.savedAt) > AUTO_DIAL_SESSION_MAX_AGE_MS) {
      clearAutoDialSession();
      return null;
    }
    return parsed;
  } catch {
...
async function runCallPipeline(
  ...
  try {
    ...
    onUpdate({ status: 'calling', callSid, conversationId });
...
export default function CallCenter() {
  ...
  const scriptCacheRef = useRef<Record<string, string>>({}); // Cache scripts per customer+caller+gender+strategy
  ...
  const autoDialSessionRestoredRef = useRef(false);
  ...
  const [scriptReadyKey, setScriptReadyKey] = useState<string | null>(null);
  const conversationScrollRef = useRef<HTMLDivElement | null>(null);

  const getScriptCacheKey = useCallback((customer: DbCustomer, caller = autoSelectCaller(customer), settings = loadSettings()) => {
    const promptVersion = String(settings.geminiSystemPrompt || '').length;
    return [
      customer.id,
      customer.assigned_agent || '',
      customer.assigned_tone || '',
      String(customer.gender || 'male').toLowerCase(),
      caller.id || caller.name || '',
      String(caller.gender || 'female').toLowerCase(),
      caller.voice || '',
      String(settings.maxPtpDays || 5),
  ...
  const syncEndedCallUi = useCallback((conv: any, customerName: string) => {
    ...
    const terminalReason = String(conv?.terminal_reason || '').toLowerCase();
    ...
    if (!finalResponse) {
      if (normalizedStatus === 'busy') {
        ...
        finalResponse = terminalReason === 'unreachable_or_switched_off' ? 'switched_off' : 'no_answer';
      } else if (['no_answer', 'no-answer', 'canceled', 'cancelled'].includes(normalizedStatus)) {
    ...
    // Unpin the finished customer so queue reflects only pending customers.
    // Concluded call remains visible in Call History via local + DB history merge.
  ...
  useEffect(() => {
    if (!current) return;
    const settings = loadSettings();
    const caller = autoSelectCaller(current);
    const cacheKey = getScriptCacheKey(current, caller, settings);
    if (scriptCacheRef.current[cacheKey]) {
      setAiScript(scriptCacheRef.current[cacheKey]);
      setScriptReadyKey(cacheKey);
    } else if (current) {
      setAiScript(null);
      setScriptReadyKey(null);
    }
  }, [current?.id, current?.assigned_agent, current?.assigned_tone, current?.gender, getScriptCacheKey]);
  ...
  useEffect(() => {
    if (autoDialSessionRestoredRef.current) return;
    autoDialSessionRestoredRef.current = true;
    const persisted = loadAutoDialSession();
    if (!persisted) return;

    setAutoDialJobs(persisted.jobs || {});
    setAutoDialRunning(Boolean(persisted.running));
  }, []);

  useEffect(() => {
    const hasJobs = Object.keys(autoDialJobs).length > 0;
    if (!autoDialRunning && !hasJobs) {
      clearAutoDialSession();
      return;
    }
    saveAutoDialSession({
      running: autoDialRunning,
      jobs: autoDialJobs,
      savedAt: Date.now(),
    });
  }, [autoDialJobs, autoDialRunning]);

  useEffect(() => {
    if (!autoDialRunning || Object.keys(autoDialJobs).length === 0) return;

    const allTerminal = Object.values(autoDialJobs).every((j) => ['done', 'failed'].includes(j.status));
    if (allTerminal) {
  ...
  useEffect(() => {
    if (!autoDialRunning) return;

    const activeJobs = Object.values(autoDialJobs).filter((j) =>
      ['calling', 'analyzing'].includes(j.status) && !!j.conversationId
    );
    ...
    const syncJobStatuses = async () => {
      ...
      for (const job of activeJobs) {
        ...
        const { data, error } = await fetchConversationStatus(job.conversationId);
        if (cancelled || error || !data) continue;

        const normalizedStatus = String(data.status || '').toLowerCase();
        if (normalizedStatus === 'ending') {
          setAutoDialJobs((prev) => ({
            ...prev,
            [job.customerId]: {
              ...prev[job.customerId],
              status: 'analyzing',
        ...
        if (!terminalStatuses.has(normalizedStatus)) continue;
        ...
        setAutoDialJobs((prev) => ({
          ...
          [job.customerId]: {
            ...prev[job.customerId],
            status: 'done',
            outcome: (data.final_response || 'unknown').replace(/_/g, ' '),
            duration: data.duration || prev[job.customerId]?.duration,
            conversationId: job.conversationId,
          },
        }));
      }
    ...
    void syncJobStatuses();
    const interval = setInterval(() => { void syncJobStatuses(); }, 3000);
  ...
  useEffect(() => {
    ...
    pollConversationStatus(
      ...
      if (result.data) {
        ...
        // Polling timed out — re-check once and avoid synthetic no-answer while call may still be ringing/active.
        void (async () => {
          const latest = await fetchConversationStatus(conversationId);
          if (cancelled || terminalHandled) return;

          const latestStatus = String(latest.data?.status || '').toLowerCase();
          if (latestStatus === 'ending') {
            syncEndingCallUi();
            return;
          }

          if (terminalStatuses.has(latestStatus)) {
            handleTerminal(latest.data);
            return;
          }

          if (['queued', 'initiated', 'ringing', 'active', 'in-progress', 'in_progress'].includes(latestStatus)) {
            toast({
              title: 'Still waiting for terminal status',
              description: `Call with ${customerName} is ${latestStatus}. Keeping live status until backend finalizes.`,
            });
            return;
          }

          syncEndedCallUi(
            { status: 'failed', final_response: 'failed', notes: 'Call status poll timeout — backend did not return terminal state' },
            customerName,
          );
        })();
      }
  ...
  useEffect(() => {
    const settings = loadSettings();
    const shouldTrackLiveCall = !settings.testingMode && !!conversationId && (callActive || analyzing || conversationPhase === 'ending');
    if (!shouldTrackLiveCall || !conversationId) return;
    ...
    try {
      const wsBase = import.meta.env.VITE_API_URL || `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}`;
      const wsUrl = wsBase.replace(/^http/i, 'ws') + '/dashboard';
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        void (async () => {
          ...
          const statusPayload = latest.data || {
            status: normalizedStatus,
            final_response: msg?.final_response || null,
            terminal_reason: msg?.terminal_reason || null,
            notes: msg?.terminal_reason ? `Terminal reason: ${msg.terminal_reason}` : '',
          };

          syncEndedCallUi(statusPayload, current?.name || 'Customer');
        })();
      } catch {
      }
    };
  ...
  const generateScript = async (targetCustomer = current) => {
    ...
    const requestKey = getScriptCacheKey(targetCustomer, caller, settings);
    latestScriptRequestRef.current = requestKey;
    setScriptReadyKey(null);
    setScriptLoading(true);
    if (current?.id === targetCustomer.id) setAiScript(null);
    try {
      const { data, error } = await invokeFunction('generate-script', {
      ...
      if (latestScriptRequestRef.current !== requestKey) return;
      if (error) throw error;
      if (data?.script) {
        if (current?.id === targetCustomer.id) {
          setAiScript(data.script);
        }
        scriptCacheRef.current[requestKey] = data.script;
        return data.script as string;
      } else if (data?.error) {
        toast({ title: 'Script Error', description: data.error, variant: 'destructive' });
      }
    } catch (err: any) {
      if (latestScriptRequestRef.current === requestKey) {
        toast({ title: 'Script Generation Failed', description: err.message, variant: 'destructive' });
      }
    } finally {
      if (latestScriptRequestRef.current === requestKey) {
        setScriptReadyKey(requestKey);
  ...
  const startCall = async (targetCustomer = current) => {
    ...
    const settings = loadSettings();
    const caller = autoSelectCaller(targetCustomer);
    const cacheKey = getScriptCacheKey(targetCustomer, caller, settings);

    const cachedScript = scriptCacheRef.current[cacheKey] || null;
    const currentCustomerScript = scriptReadyKey === cacheKey && current?.id === targetCustomer.id
      ? aiScript
      : null;
  ...
  const startAutoDial = useCallback(() => {
    ...
    const safeConcurrent = 1; // Safety: force one-by-one sequential dialing for consistent queue UI
    ...
    toast({ title: `⚡ Auto-Dial Starting in 5s...`, description: `${batch.length} customers (priority top→bottom), sequential one-by-one` });
